...
| `GET /api/firmalar` · `POST /api/firmalar` | **V3-C Aşama 2.1** — firma çekirdeği (`suppliers`, 0036). `POST {ad, varsayilan_dil?: tr\|en\|zh, ulke?, platform?, whatsapp?, eposta?, notlar?, varsayilan_gecerlilik_gun?}` → 201 firma. Firma bir KAYITTIR, hesap değil (K103); Firmalar & Kişiler modülü ilerideki fazda |
...
| `GET /api/sablonlar` | **V3-C Blok E** — liste şablonları (0039 `list_templates`): `[{id, ad, aciklama, urun_sayisi, ornek_urunler[3], kaynak_list_id, kullanim_sayisi, son_kullanim_at, created_at, updated_at}]`, kullanım sırasına göre |

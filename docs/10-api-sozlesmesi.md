...
| `GET /api/media/proxy?url=` | v1.2.2 D2 — K47 GÖRSEL VEKİLİ (Auth; GET olduğu için CSRF muaf). Uzak ana görsel kuyruk indirene kadar buradan çizilir (alicdn Referer ACL tarayıcıda boş kare bırakır). İndirme hattıyla AYNI denetim (`UrlGuard`: yalnız https, `MEDIA_ALLOWED_HOSTS`, açık ağ adresi, DNS pinleme) + GERÇEK görsel imzası. → 200 görsel baytları (`Content-Type` gerçek mime, `Cache-Control: private, max-age=3600`, `nosniff`, `noindex`); `url` yoksa 422 `VALIDATION`; beyaz liste/güvenlik reddi 403 `FORBIDDEN` (sabit mesaj, K51); gövde görsel değilse 415 `UNSUPPORTED_MEDIA`; kaynak/ağ hatası 502 `UPSTREAM_ERROR`. Diske yazmaz, yeniden kodlamaz |
| — ürün nesnesi (v1.2.2 D1/D2) | Ürün nesnesine üç TÜRETİLMİŞ alan eklendi (kolon değil): `main_image_uzak` (bool — ana görsel henüz arşive alınmadı), `main_image_gosterim` (string\|null — panelin ÇİZECEĞİ adres: yerelse kendisi, uzaksa `/api/media/proxy?url=…`), `media_pending` (bool — medya işi kuyrukta bekliyor/çalışıyor; `jobs` tablosu yoksa false). Yakalama artık HİÇBİR görsel indirmez: `main_image` kaynak URL ile yazılır, kuyruk ana görseli + galeriyi indirip satır CAS'ıyla sonlandırır. `GET /api/system/queue` yanıtına `ertelenen` (int — bütçe/koşul yüzünden sonraki tura kalan iş; hata DEĞİL) ve `devre_kesici` (`{tur, kapanma_at, esik, dakika}`\|null) eklendi; `GET /api/system/status` yanıtına `kuyruk_devre_kesici` (aynı şekil\|null) eklendi |

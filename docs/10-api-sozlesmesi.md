...
| `GET /api/settings/glossary?lang=zh\|en` | İE#14 A2 (K56 Katman 1) — Ayarlar > Terminoloji. 200 `{lang, writable, terms:{kaynak: karşılık}}` + `meta.languages:['zh','en']`. Sözlük DOSYADADIR (`config/sozluk-<dil>-tr.php`), veritabanında değil — migration gerektirmez |
| `PUT /api/settings/glossary` | `{lang, terms:{kaynak: karşılık}}` — listenin TAMAMI yazılır (ekle/düzelt/sil tek uçtan). En çok 5000 terim; terim ≤120, karşılık ≤200 karakter; sınırı aşan satır sessizce elenir. `config/` yazılamıyorsa 500 `SERVER_ERROR` (izin sorunu gizlenmez). Değişiklik `glossary_updated` olarak aktiviteye yazılır |

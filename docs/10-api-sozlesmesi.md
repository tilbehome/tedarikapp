...
| Uç | Açıklama |
|---|---|
| `GET /api/lists` | Filtre: `visibility=active\|passive\|archived`, `status`, `q` (ad/tedarikçi içinde arama), **`sekme`** (V3-C Blok E: `hazirlaniyor\|fiyat_bekleniyor\|degerlendirmede\|onayli\|tamamlandi\|iptal`). Her liste **türetilmiş** alanlar taşır: `sekme` (durum + teklif turları; saklanmaz), `fiyatlama {fiyatlanan, toplam, yuzde, kaynak: tur\|urun}` ("18/25 fiyatlandı"), `tur_ozeti` (açık tur: id, tur_no, state, sent_at, first_viewed_at, responded_at, valid_until) ve `saglik[]` (`fiyat_bekleyen`, `cikti_guncel_degil`, `kur_sapmasi` ≥%2, `teklif_suresi` ≤48 sa). `meta.sayimlar` (sekme başına, FİLTRESİZ küme) ve `meta.kpi {fiyat_bekleyen_liste, karar_bekleyen_liste, suresi_dolan_teklif, fiyatlanmayan_satir}` |
...
| `GET /api/firmalar` · `POST /api/firmalar` | **V3-C Aşama 2.1** — firma çekirdeği (`suppliers`, 0037). `POST {ad, varsayilan_dil?: tr\|en\|zh, ulke?, platform?, whatsapp?, eposta?, notlar?, varsayilan_gecerlilik_gun?}` → 201 firma. Firma bir KAYITTIR, hesap değil (K103); Firmalar & Kişiler modülü ilerideki fazda |
| `GET /api/lists/{id}/turlar` · `POST /api/lists/{id}/turlar` | **V3-C Aşama 2.1** — listenin teklif turları (`liste × firma × tur`). `POST {firma_id, gecerlilik_gun?, portal_dili?}` → 201 tur (`state=DRAFT`, `tur_no` otomatik artar, KUR DÖRTLÜSÜ açılış anında KOPYALANIR — K104). 422 `TUR_ACIK` (aynı firmaya açık tur var), `LISTE_BOS`, `LISTE_KAPALI`; 404 `FIRMA_YOK` |
| `GET /api/turlar/{id}` | Tur + (gönderildiyse) donmuş `rfq_satirlari[]` (`rfq_satir_id` UUID, `urun_adi {tr,en,zh}`, `kaynak_urun`, `talep_miktar`, `talep_birim`, `gorsel_url`). Her tur `etiket` taşır ("R2 gönderildi" — tur numarası durum adına gömülmez, #15 §2), `kur {para_birimi, deger, kaynak, kilit_at}`, `goruntulendi` (GÖZLEM; sahip yazamaz), `bekleme_gun` |
| `POST /api/turlar/{id}/gonder` | **DRAFT → SENT, tek transaction, geri alınamaz.** `{gecerlilik_gun?, kanal?: whatsapp\|eposta\|panel\|diger, alici?}`. Üç şey donar: RFQ snapshot (`rfq_snapshots`+`rfq_lines`), kur kilidi (`kur_kilit_at`), TUR kimlikli paylaşım (`shares.supplier_round_id`) + 6 haneli anahtar + gönderim günlüğü. Yanıt: tur + `share_url`, `share_token` (64 hex) ve `erisim_anahtari` — **yalnız bu yanıtta** (K51); DB'de hash + şifreli kopya. Geçersiz geçiş 422 `TUR_GECIS` |
| `POST /api/turlar/{id}/onayla` · `POST /api/turlar/{id}/vazgec` · `POST /api/turlar/{id}/revizyon` | Sahip kararları (durum makinesi `TurDurumMakinesi`, CAS yazım). `onayla`: RESPONDED → APPROVED. `vazgec {sebep?}`: açık durumdan ABANDONED, tur paylaşımı iptal edilir. `revizyon {sebep (zorunlu), rate_policy?: inherit\|refresh}` → 201 **YENİ tur** (`tur_no+1`, `parent_round_id`, DRAFT); eski tur REVISION_REQUESTED. `inherit` eski kur dörtlüsünü kopyalar. Sahibin elle durum yazma yolu YOKTUR (VIEWED yazılamaz) |
| `GET /api/teklifler` | Teklifler modülü: `{acik: Tur[], gecmis: Tur[]}` — açık = nihai olmayan (DRAFT…RESPONDED, EXPIRED), geçmiş = APPROVED/ABANDONED/REVOKED/REVISION_REQUESTED. Ana kolon `goruntulendi` + `bekleme_gun` (yol haritası §7.6) |
| `GET /api/lists/{id}/gonderim-gunlugu` | `share_dispatch_log`: `[{id, supplier_round_id, kanal, alici, dil, token_prefix, created_at}]` — hangi link kime ne zaman hangi kanaldan gitti |
| `GET /api/turlar/{id}/yanit` | **V3-C Aşama 2.2** — turun taslak firma yanıtı: `{tur_id, state, satirlar: {rfq_satir_id → kanonik satır}}`. Kanonik satır `quote_lines` adlarıyla birebirdir (`yanit_durumu`, `ddp_birim_fiyat`, `para_birimi`, `ddp_kdv_dahil_onayi`, `moq_deger/birim`, `termin_baslangici/_aciklamasi/_suresi/_birimi`, `koli_*`, `ambalaj`, `firma_notu`, `alternatif_baglanti/_aciklama`, `kademeler[{min_adet,max_adet,birim_fiyat,para_birimi,kademe_tipi}]`); para/miktar DİZE (K14) |
| `POST /api/turlar/{id}/yapistir-ayristir` | `{metin}` (≤200 KB) → ÖNİZLEME, **yazmaz**: `{parmak_izi, satirlar[{rfq_satir_id, urun_kodu, urun_adi, talep_miktar, eslesme, yeni, eski, hatalar[], eksik_zorunlu[], secilebilir, varsayilan_secili}], belirsiz[{parca, aday_satir_idleri, neden, yasak_otomatik_islem}], dogrulama_hatalari[], eslesmeyen_satirlar[]}`. Eşleştirme anahtarı ÜRÜN KODUDUR; ad ile eşleşme yalnız tek adayda; belirsiz parça hiçbir satıra yazılmaz (altın set kabul kapısı: %0 yanlış ürün). 422 `TUR_GONDERILMEMIS` |
| `POST /api/turlar/{id}/yanit-uygula` | `{kaynak: yapistir\|excel, parmak_izi (64 hex, önizlemeden), etiket?, satirlar[kanonik + temizle?: string[]]}` → `{tekrar, yazilan, satirlar[], state, yanit}`. Tek transaction; aynı `parmak_izi` ikinci kez **yazmaz** (`tekrar=true`). Boş alan mevcut değeri SİLMEZ; silme yalnız `temizle` listesiyle. Tek satırda alan hatası varsa HİÇBİRİ yazılmaz: 422 `YANIT_GECERSIZ` + `meta.hatalar[{satir_id, alan, deger, kural}]`. 422 `TUR_KILITLI` (yanıt kabul etmeyen durum), `SATIR_YABANCI`, `SATIR_MUKERRER`, `SATIR_YOK`. Başarıda tur SENT/VIEWED → PRICING; aktivite `quote_imported` (`actor=product_owner source=paste_parse\|excel_import`) |
| `POST /api/turlar/{id}/excel-sablon` | `{dil?: tr\|en\|zh}` → `.xlsx` (CSRF'li POST indirme). Beş sayfa: START · QUOTATION (A–AF, spec §4) · PRICE_TIERS · VALIDATION (çok gizli) · MANIFEST (çok gizli: `schema_version, exported_at, supplier_round_id, rfq_snapshot_id, row_count, manifest_signature` + satır imzaları). Dosyada link/anahtar/sır YOK; mevcut taslak yanıt önceden dolu gelir. 422 `TUR_GONDERILMEMIS` |
| `POST /api/turlar/{id}/excel-ice-aktar` | `{dosya_base64}` (≤3 MB) → ÖNİZLEME, **yazmaz**: `{parmak_izi, manifest, ozet{uygulanabilir,uyarili,hatali,belirsiz,degisiklik_yok}, satirlar[{rfq_satir_id, hucre, grup, secilebilir, varsayilan_secili, imza_bozuk, eski, yeni, degisen[], hatalar[], uyarilar[], belirsiz[]}]}`. Fail-closed sıra (spec §7): güvenlik (makro/dış bağlantı/OLE/şifreli → 422 `DOSYA_GUVENLIK`; zip değil → `DOSYA_BICIM`) → `DOSYA_SEMA` → başka tur `DOSYA_YABANCI` → snapshot farkı `DOSYA_ESKI` → manifest imzası `DOSYA_IMZA` → satır: YABANCI/MÜKERRER/imza bozuk hatali; formül çalıştırılmaz; boş hücre = değişiklik yok; para birimsiz fiyat = belirsiz |
| `POST /api/turlar/{id}/excel-sonuc` | `{onizleme (+uygulanan?: string[])}` → `…-IMPORT-RESULT.xlsx` (satır · grup · uygulandı mı · değişen · hatalar · uyarılar · belirsiz · yeni değer özeti). Durumsuz: önizleme JSON'u geri verilir; hatalı dosyaya doğrudan yazılmaz (spec §10) |
| `GET /api/sablonlar` | **V3-C Blok E** — liste şablonları (0040 `list_templates`): `[{id, ad, aciklama, urun_sayisi, ornek_urunler[3], kaynak_list_id, kullanim_sayisi, son_kullanim_at, created_at, updated_at}]`, kullanım sırasına göre |
| `POST /api/lists/{id}/sablon` | `{ad, aciklama?}` → 201 şablon: listenin ürün kümesi DONDURULUR (`urun_json`; takip kodu taşınmaz). Şablon listeye bağlı değildir. 422 `LISTE_BOS` |
| `POST /api/sablonlar/{id}/liste` | `{name?, period?, supplier_name?}` → 201 TASLAK liste (günün kuru, ürünler `to_order`), tek transaction; `kullanim_sayisi`++ · `PATCH /api/sablonlar/{id}` `{ad?, aciklama?}` · `DELETE /api/sablonlar/{id}` → 204 (kalıcı; geri alma penceresi panelde, K105 §2.6 ertelenmiş kip) |
| `GET/POST /api/gorunumler/{ekran}` · `DELETE /api/gorunumler/{ekran}/{ad}` | K105 §2.3 — ekran başına kaydedilmiş görünüm (`kesif.gorunumler` deseni, genel uç; ekran allow-list: `listeler`). `POST {ad, sorgu{…}, varsayilan?}`; aynı ad üzerine yazar, varsayılan tek, en çok 30 |
...
| `POST /api/panel/translate-suggest` | İE#13 C4 (K54) — `{text}` (1..500 karakter) → 200 `{suggestion: string|null, cached: bool, provider: string|null, source: 'sozluk'|'makine'|null, is_suggestion: true}` *(İE#14 A2: `source` önerinin hangi katmandan geldiğini söyler; `sozluk` belirlenimcidir ve `TRANSLATE_ENABLED=0` iken bile döner)*. **Öneri yoksa da 200 döner** (`suggestion: null`) — çeviri akışın zorunlu parçası değildir, istemci sessizce geçer. Boş/çok uzun metin → 422 `VALIDATION`. Yanıt hiçbir ürün alanını DEĞİŞTİRMEZ; yazma yalnız kullanıcı "Kullan" dedikten sonra normal ürün güncelleme ucuyla olur (K54) |
| `GET /api/settings/glossary?lang=zh\|en` | İE#14 A2 (K56 Katman 1) — Ayarlar > Terminoloji. 200 `{lang, writable, terms:{kaynak: karşılık}}` + `meta.languages:['zh','en']`. Sözlük DOSYADADIR, veritabanında değil — migration gerektirmez. Okuma: `config/sozluk-<dil>-tr.php` (depoyla gelen varsayılan) + `storage/sozluk-<dil>-tr.php` (panel üstyazımı, öncelikli) |
| `PUT /api/settings/glossary` | `{lang, terms:{kaynak: karşılık}}` — listenin TAMAMI yazılır (ekle/düzelt/sil tek uçtan). En çok 5000 terim; terim ≤120, karşılık ≤200 karakter; sınırı aşan satır sessizce elenir. Yazma **yalnız `storage/`** altınadır (K44); `config/` dosyasına dokunulmaz. `storage/` yazılamıyorsa 500 `SERVER_ERROR` (izin sorunu gizlenmez). Değişiklik `glossary_updated` olarak aktiviteye yazılır |
| `POST /api/panel/translate-product` | İE#14 A2 (K56 Katman 2) — ürünün TAMAMI tek çağrıda: `{name?, category?, attributes?{}, variants?[], source_lang?}` → 200 aynı yapı + `meta:{provider, sources:{alan: 'sozluk'\|'makine'\|'ham'}}`. Yanıt yalnız ÖNERİDİR, hiçbir alan yazılmaz (K54). Öznitelik ADLARI da çevrilir (品牌 → Marka) |
...
Belge kodu ve revizyon (F7 · **İE#14 B1 / K57 ile düzeltildi**): kod `TDK-<yıl>-<liste no>` + `· Rev <harf>`; harf **listenin SÜRÜMÜNDEN** türer (`lists.revision`), indirme sayısından DEĞİL. Aynı içerikten alınan Excel ve PDF AYNI harfi taşır; harf ancak içerik değişince ilerler (ürün ekleme/silme/güncelleme, ürün durumu, liste durumu, kur kilidi). Rev A dışındaki her belge "bu belge aynı listenin önceki çıktılarını GEÇERSİZ KILAR" ibaresi taşır. Snapshot `options` bloğu bu seçimleri saklar — geçmişten indirme AYNI belgeyi üretir.

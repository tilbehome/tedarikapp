...
- `config/durumlar.json` repo tek kaynağına göre yalnız “Sipariş Verildi” aşaması `liste.ordered` ile birebir örtüşür. Diğer aşamalarda yakın anlamlı kodlara zorlamalı eşleme yapılmaz; karşılık yoksa `durum_kod_ref=null` kalır ve İE'de kod eklenmesi gerekir.
...
| Sıra | Görünür durum | `durum_kod_ref` | Kod bağlama notu | `terim_ref` | `terim_onerisi` (anahtarsız) | İş anlamı |
|---:|---|---|---|---|---|---|
| 5 | Sipariş verildi | `liste.ordered` | `durumlar.json` birebir karşılığı | `null` | TR: Sipariş verildi · EN: Order placed · ZH: 订单已下达 | Onaylı kalem tahsislerinden sipariş snapshot'ı oluştu. |
| 6 | Üretim/tedarikte | `null` | `durumlar.json`'da karşılık yok — İE'de kod eklenmesi gerekir | `null` | TR: Üretim/tedarikte · EN: In production/sourcing · ZH: 生产/备货中 | Firma üretim veya tedarik sürecini başlattı. |
| 7 | Çin limanında | `null` | `durumlar.json`'da karşılık yok — İE'de kod eklenmesi gerekir | `null` | TR: Çin limanında · EN: At China port · ZH: 已到中国港口 | Yük liman/terminal teslim kanıtıyla ihracat ayağına ulaştı. |
| 8 | Gemide (ETA) | `null` | `durumlar.json`'da karşılık yok — İE'de kod eklenmesi gerekir | `null` | TR: Gemide · EN: On vessel · ZH: 已装船 | Konşimento/yükleme kanıtı var; ETA ayrı değişebilir alandır. |
| 9 | Türkiye limanında | `null` | `durumlar.json`'da karşılık yok — İE'de kod eklenmesi gerekir | `null` | TR: Türkiye limanında · EN: At Türkiye port · ZH: 已到土耳其港口 | Yalnız takip etiketi; fiilî varış kanıtına dayanır. |
| 10 | Teslim edildi | `null` | `durumlar.json`'da karşılık yok — İE'de kod eklenmesi gerekir | `null` | TR: Teslim edildi · EN: Delivered · ZH: 已交付 | Fiziksel teslim kaydı oluşur ve mal kabul süreci açılır. |
| terminal | Kapandı | `null` | `durumlar.json`'da karşılık yok — İE'de kod eklenmesi gerekir | `null` | TR: Kapandı · EN: Closed · ZH: 已关闭 | Mal kabul/rücu/ödeme açık işi kalmadığında salt okunur olur. |
...
4. `config/durumlar.json` — repo tek kaynağı; `liste.ordered` birebir bağlandı, karşılığı olmayan altı aşama için İE kod gereksinimi kaydedildi.

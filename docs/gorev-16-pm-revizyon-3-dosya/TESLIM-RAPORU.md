...
| 5 | `landed-cost-kalibrasyon.json` | 16C | 1.366 | 6 masraf türü, 3 dağıtım anahtarı, 16 LC vakası; 4 tuzaklı vaka |
| 6 | `cbm-matematik.json` | 16D | 499 | 4 taşıma tipi, 14 CBM vektörü; hacim/ağırlık ve yer-kaldı çıktıları |
| 7 | `rucu-pdf-metinleri.json` | 16E | 519 | 59 benzersiz `belge.rucu.*` sabit metni; TR/EN/ZH |

Toplam: **3.638 satır**. Bu toplam raporun kendisini içermez.
...
1. `config/durumlar.json` repo tek kaynağıyla bağlama tamamlandı: “Sipariş Verildi” → `liste.ordered`; diğer altı aşamanın birebir karşılığı yoktur ve İE'de kod eklenmesi gerekir.

...
## Ön Koşul
- Bünyamin, Claude'un (PM) hazırladığı 11 MD dosyasını (README.md + CLAUDE.md + 00–08 belgeleri) proje klasörünün yanına `docs-taslak/` klasörü olarak koymuş olacak.
...
2. Klasör iskeletini oluştur (içleri şimdilik boş, `.gitkeep` ile):
   ...
   docs/is-emirleri/
   ```
3. `docs-taslak/` içindeki dosyaları yerleştir: `README.md` ve `CLAUDE.md` repo köküne, `00-…md` – `08-…md` dosyaları `docs/` altına. Bu iş emri dosyasını `docs/is-emirleri/is-emri-01.md` olarak kaydet. CLAUDE.md'yi OKU — bundan sonraki tüm oturumlarında bağlayıcıdır.
...
- [ ] 9 belge `docs/` altında + README ve CLAUDE.md kökte, linkler çalışıyor.
